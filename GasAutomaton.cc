...
inline cv::Vec3b rgb(Automata::ubyte r, Automata::ubyte g, Automata::ubyte b) {
  return {b, g, r};
}
Automata::GasAutomaton::GasAutomaton(size_t totalSteps, std::string outputPath)
    : CellularAutomata(totalSteps), outputPath(outputPath) {
...
void Automata::GasAutomaton::applyRule() {
  for (unsigned x = 0; x < GasAutomaton::size; ++x) {
    unsigned wneigh = x == 0 ? (GasAutomaton::size - 1) : x - 1,
        eneigh = x == GasAutomaton::size - 1 ? 0 : x + 1;
    for (unsigned y = 0; y < GasAutomaton::size; ++y) {
      unsigned nneigh = y == 0 ? GasAutomaton::size - 1 : y - 1,
          sneigh = y == GasAutomaton::size - 1 ? 0 : y + 1;
      bool n = get<North>(x, y),
          s = get<South>(x, y),
          e = get<East>(x, y),
          w = get<West>(x, y),
          sol = get<Solid>(x, y);
